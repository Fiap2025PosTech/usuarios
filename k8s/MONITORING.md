...
# URL da API
kubectl get svc users-service -n users

# URL do Prometheus
kubectl get svc prometheus -n users

# URL do Grafana
kubectl get svc grafana -n users
```
...
rate(http_requests_received_total{namespace="users"}[5m])

# Latência p95
histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{namespace="users"}[5m]))

# Uso de memória
process_working_set_bytes{namespace="users"} / 1024 / 1024

# CPU usage
rate(process_cpu_seconds_total{namespace="users"}[5m]) * 100

# Requisições por status code
sum by (code) (rate(http_requests_received_total{namespace="users"}[5m]))
```
...
O dashboard "Usuarios API - AWS EKS" já está provisionado automaticamente com:
- Taxa de requisições HTTP
- Latência (p95)
- Status codes
...
# 4. Verificar status
kubectl get pods -n users -l app=prometheus
kubectl get pods -n users -l app=grafana

# 5. Aguardar LoadBalancers
kubectl get svc -n users -w
```
...
kubectl get sa prometheus -n users
...
kubectl exec -it -n users deployment/grafana -- cat /etc/grafana/provisioning/datasources/datasources.yaml

# Testar conectividade
kubectl exec -it -n users deployment/grafana -- wget -O- http://prometheus:9090/-/healthy
```
...
```bash
# Verificar eventos
kubectl get events -n users --sort-by='.lastTimestamp' | grep LoadBalancer

# Verificar service
kubectl describe svc prometheus -n users
kubectl describe svc grafana -n users
...
kubectl delete sa prometheus -n users
```
...
```bash
kubectl set env deployment/grafana -n users GF_SECURITY_ADMIN_PASSWORD=SUA_SENHA_SEGURA
```
